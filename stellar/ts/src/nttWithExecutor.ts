 ...
 * `InvokeHostFunction`, so — unlike Sui's PTB or an EVM multicall — the manager
 * transfer and `request_execution` cannot be batched client-side. Everything it
 * does runs under the single auth tree rooted at the sender, and it takes no
 * custody of its own.
 *
 * The executor contract is bound into the wrapper at construction, so a caller
 * only ever names the wrapper; the manager is a per-call argument, which is why
 * one wrapper serves every manager on the chain.
 */
  ...
  implements NttWithExecutor<N, C>
{
   ...
   * localnet or a private deployment names its own wrapper — the table has no
   * entry until a public deployment exists, and an invented address is worse
   * than none.
   */
  constructor(
    readonly network: N,
   ...
   * rest, so `quote.referrerFee` / `quote.remainingAmount` are the route's
   ...
   * read slightly high.
   *
   * Every token movement — the referrer fee, the manager's lock/burn, and the
   * executor's XLM fee — is authorized by `sender` alone, which is also the
   * transaction source, so the envelope signature covers the whole auth tree
   * and no auth entry has to be signed separately.
   *
  ...
  async *transfer(
    ...
    // Checked here so a stale quote is named as such, rather than arriving as
    ...
    // The fee is paid on Stellar in the transfer token, with a plain
    // `token.transfer`, so the referrer has to be an address this chain can
    // name. A Stellar `UniversalAddress` is the one-way `hash_address` (D2) and
    // is no help, so require the native form rather than trying to invert it.
    ...
    yield await ntt.prepare(
      ...
      new Contract(wrapper).call(
        ...
        structArg({
          ...
          // own address type decides its wire form.
          recipient: bytesArg(
  ...
  /**
   * Zero on both counts, and neither is a rounded-down estimate.
   *
   * This is asked of the *destination* chain's binding, to fill the executor's
   * `GasInstruction` for the delivery. Delivering on Stellar is a Soroban
  ...
  /**
   * The executor's `payee`, from the 32 raw bytes the signed quote carries at
   * `quote[24..56]`.
   *
   ...
   * bytes are a contract id if a contract instance is deployed at them —
   ...
   * not exist cannot be credited, so the transfer fails in simulation instead
   * of paying into a balance nobody holds the key to.
   */
  ...
  /** The wrapper to invoke, resolved only where one is actually needed. */
